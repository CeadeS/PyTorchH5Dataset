...
def collate_samples_tensor(sample):
    ...
    if isinstance(meta[0], Tensor) or isinstance(meta[0], np.ndarray):
        ...
        if data.get_worker_info() is not None:
            ...
            out = sample[0].new(storage).view(-1, *list(sample[0].size())[1:])
            #out = sample[0].new(storage).view(-1, *list(sample[0].size()))
            #out = sample[0].new(storage)

        return cat(sample, dim=0, out = out), (cl, meta)
    else:
        meta = concat(meta)
...
class DataLoader(object):
    ...
    def __iter__(self):
        for sample, (meta_class, meta_indices) in self.dataloader:
            if self.pin_memory and isinstance(sample, Tensor) and not sample.is_cuda:
